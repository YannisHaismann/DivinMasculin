...
export const Avis = ({
  ...
  return (
    <Fragment>
      {items.map((item) => (
        <div
          className="-mr-5 relative group shrink-0"
          key={item.name}
        ...
        >
          ...
          <Image
            ...
            src={item.image}
            alt={item.name}
            className="object-cover !m-0 !p-0 object-top rounded-full size-12 border-2 border-zinc-200 group-hover:scale-105 group-hover:z-30 relative transition duration-500"
          />
        </div>
      ))}
    </Fragment>
